...
MPU9250K::MPU9250K(uint8_t intPin)
{
  uint8_t _intPin = intPin;
}


uint8_t MPU9250K::getMPU9250ID(uint8_t MPUnum)
{
  uint8_t c = readByte(MPUnum, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
  return c;
}

  uint8_t MPU9250K::getAK8963CID(uint8_t MPUnum)
{
...
float MPU9250K::getMres(uint8_t Mscale) {
  switch (Mscale)
  {
...
float MPU9250K::getGres(uint8_t Gscale) {
  switch (Gscale)
  {
...
float MPU9250K::getAres(uint8_t Ascale) {
  switch (Ascale)
  {
...
void MPU9250K::accelWakeOnMotion(uint8_t MPUnum)
{
...
void MPU9250K::gyromagSleep(uint8_t MPUnum)
{
  uint8_t temp = 0;
  writeByte(MPUnum, I2C_SLV0_ADDR, AK8963_ADDRESS | 0x80);        // Set the I2C slave address of AK8963 and set for read.
...
void MPU9250K::gyromagWake(uint8_t MPUnum, uint8_t Mmode)
{
  uint8_t temp = 0;
  writeByte(MPUnum, I2C_SLV0_ADDR, AK8963_ADDRESS | 0x80);        // Set the I2C slave address of AK8963 and set for read.
  writeByte(MPUnum, I2C_SLV0_REG,  AK8963_CNTL);                  // I2C slave 0 register address from where to begin data transfer
...
void MPU9250K::resetMPU9250(uint8_t MPUnum)
{
  // reset device
...
void MPU9250K::readMPU9250Data(uint8_t MPUnum, int16_t * destination)
{
...
void MPU9250K::readAccelData(uint8_t MPUnum, int16_t * destination)
{
...
void MPU9250K::readGyroData(uint8_t MPUnum, int16_t * destination)
{
...
bool MPU9250K::checkNewMagData(uint8_t MPUnum)
{
  bool test;
...
bool MPU9250K::checkNewAccelGyroData(uint8_t MPUnum)
{
  bool test;
...
bool MPU9250K::checkWakeOnMotion(uint8_t MPUnum)
{
  bool test;
...
void MPU9250K::readMagData(uint8_t MPUnum, int16_t * destination)
{
...
int16_t MPU9250K::readGyroTempData(uint8_t MPUnum)
{
...
void MPU9250K::initAK8963Slave(uint8_t MPUnum, uint8_t Mscale, uint8_t Mmode, float * magCalibration)
{
...
void MPU9250K::initMPU9250(uint8_t MPUnum, uint8_t Ascale, uint8_t Gscale, uint8_t sampleRate)
{  
 // wake up device
...
void MPU9250K::magcalMPU9250(uint8_t MPUnum, float * dest1, float * dest2) 
{
...
void MPU9250K::calibrateMPU9250(uint8_t MPUnum, float * dest1, float * dest2)
{  
...
void MPU9250K::SelfTest(uint8_t MPUnum, float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
...
// I2C read/write functions for the MPU9250 sensors
  ...
  void MPU9250K::writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
  ...
  uint8_t MPU9250K::readByte(uint8_t address, uint8_t subAddress)
{
  ...
  void MPU9250K::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest)
{  
  Wire.beginTransmission(address);   // Initialize the Tx buffer
  Wire.write(subAddress);            // Put slave register address in Tx buffer
  Wire.endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
